...
const run = () => {
  const answer = expenseReport.slice(0).reduce(
    (accumulator, currentValue, firstIndex) => {
      const inner = expenseReport.reduce(
        (accumulatorB, currentValueB, secondIndex) => {
          if (secondIndex !== firstIndex) {
            const x = expenseReport.find(
              (value, thirdIndex) =>
                value + currentValueB + currentValue === 2020 &&
